...
class Video < ApplicationRecord
  ...
  def self.fetch_trending_videos(category_id)
    ...
    api_key = ENV['API_KEY']
    url = "https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&chart=mostPopular&regionCode=JP&videoCategoryId=#{category_id}&maxResults=10&key=#{api_key}"
  ...
  def self.fetch_vtuber_videos
    Video.where(category: "VTuber").delete_all
    api_key = ENV['API_KEY']
    ...
    vtuber_keywords.each do |keyword|
      ...
      search_url = "https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&q=#{CGI.escape(keyword)}&regionCode=JP&maxResults=10&order=date&key=#{api_key}"
